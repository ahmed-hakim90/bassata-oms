...
export function StockCountWizard({
  ...
  if (step === "review") {
    return (
      <OperationalCard title="Review Variances" description="Confirm before posting">
        ...
        <ul className="max-h-96 space-y-2 overflow-y-auto">
          ...
          {varianceCount === 0 && (
            <p className="py-8 text-center text-muted-foreground">
              All counts match expected quantities
            </p>
          )}
        </ul>
        <div className="mt-6 flex gap-2">
          ...
          <Button onClick={postAdjustments} disabled={pending}>
            <Check className="size-4" /> Post Adjustments
          </Button>
        </div>
      </OperationalCard>
    );
  ...
  return (
    <OperationalCard
      title="Count Items"
      description={`${count.lines.length} products to count`}
    >
      ...
      <Button className="mt-6 w-full" onClick={saveCounts} disabled={pending}>
        <ClipboardList className="size-4" /> Review Variances
      </Button>
    </OperationalCard>
  );
}
