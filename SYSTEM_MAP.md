...
> Last updated: 2026-08-16 (purchase imports: containers + customs certificates)
...
| MOD / 03 | Cashier sessions & vault | CASH | `/sessions` (KPIs + variance chart + report links), `/sessions/[id]` |
...
| MOD / 11 | Expenses & general ledger | FINANCE | `/expenses` (category/month glance), `/accounting` (hub), `/accounting/accounts`, `/accounting/journals`, `/accounting/trial-balance`, `/accounting/ledger`, `/accounting/income-statement`, `/accounting/balance-sheet` |
...
- Quotation → sales order → sales invoice (deliver/stock) → optional credit note
- Purchase request → purchase order → partial purchase invoices → optional purchase return
...
- Settings print templates (`print_engine` at `/settings?tab=print`) → named A4 templates, layouts, block order, per-kind assignment; POS A4; delivery note (`?variant=delivery`)
