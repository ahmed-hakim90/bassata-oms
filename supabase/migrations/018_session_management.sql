-- Session management: force-close audit, RBAC permissions, session_settings
...
INSERT INTO permissions (key, label, description, group_name) VALUES
  ('session_view_all', 'View all sessions', 'View open sessions across branches', 'sessions'),
  ('session_force_close', 'Force close session', 'Force close another cashier session', 'sessions'),
  ('session_settings_manage', 'Manage session settings', 'Configure shift duration and expiry policies', 'sessions')
...
BEGIN
  FOR r_org IN SELECT id FROM organizations LOOP
    INSERT INTO role_permissions (org_id, role, permission_key) VALUES
      (r_org.id, 'owner', 'session_view_all'),
      (r_org.id, 'owner', 'session_force_close'),
      (r_org.id, 'owner', 'session_settings_manage'),
      (r_org.id, 'manager', 'session_view_all'),
      (r_org.id, 'manager', 'session_force_close'),
      (r_org.id, 'manager', 'session_settings_manage')
...
-- Defense-in-depth: reject checkout on expired sessions when blocking is enabled
