-- CafeFlow legacy cleanup: remove Souqna, online menu/orders, platform admin, monthly closing

-- ---------------------------------------------------------------------------
-- Drop triggers tied to legacy tables
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Drop legacy tables (child tables first)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS trg_monthly_closing_require_feature() CASCADE;
DROP FUNCTION IF EXISTS trg_online_orders_updated_at() CASCADE;
DROP FUNCTION IF EXISTS record_souqna_api_request(TEXT);
DROP FUNCTION IF EXISTS assert_souqna_rate_limit(TEXT);

-- Period locking removed with monthly_closes
...
-- ---------------------------------------------------------------------------
-- Drop unused product column
-- ---------------------------------------------------------------------------
ALTER TABLE products DROP COLUMN IF EXISTS publish_to_souqna;

-- ---------------------------------------------------------------------------
-- Clean viewer role permissions (CafeFlow has 4 roles only)
-- ---------------------------------------------------------------------------
DELETE FROM role_permissions WHERE role = 'viewer';

-- Strip legacy feature flags from existing orgs
UPDATE app_settings
SET value = value
  - 'monthly_closing'
  - 'online_menu'
...
UPDATE app_settings
SET value = jsonb_set(value, '{prevent_expenses_in_closed_periods}', 'false'::jsonb, true)
WHERE key = 'expense_settings'
  AND COALESCE((value->>'prevent_expenses_in_closed_periods')::boolean, false);

-- Remove online menu tokens from store settings
UPDATE stores
SET settings = settings - 'online_menu_slug' - 'online_menu_token'
WHERE COALESCE(settings, '{}'::jsonb) ?| ARRAY['online_menu_slug', 'online_menu_token'];

-- ---------------------------------------------------------------------------
-- Seed café POS categories for existing orgs (idempotent)
-- ---------------------------------------------------------------------------
INSERT INTO categories (org_id, name, sort_order, color, icon)
SELECT o.id, c.name, c.sort_order, c.color, c.icon
FROM organizations o
CROSS JOIN (
  VALUES
    ('Hot Drinks', 1, '#DC2626', 'coffee'),
    ('Cold Drinks', 2, '#2563EB', 'cup-soda'),
    ('Ice Cream', 3, '#60A5FA', 'ice-cream'),
    ('Desserts', 4, '#F472B6', 'cake'),
    ('Waffle', 5, '#F59E0B', 'grid-2x2'),
    ('Crepe', 6, '#A78BFA', 'layers'),
    ('Add-ons', 7, '#34D399', 'sparkles')
) AS c(name, sort_order, color, icon)
WHERE NOT EXISTS (
  SELECT 1 FROM categories cat
  WHERE cat.org_id = o.id AND cat.name = c.name
);

-- ---------------------------------------------------------------------------
-- Update initialize_organization for new CafeFlow orgs
-- ---------------------------------------------------------------------------
