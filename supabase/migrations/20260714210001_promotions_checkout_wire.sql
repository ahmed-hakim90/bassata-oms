-- Wire promotions into complete_checkout (+ split / expired wrappers).
-- Depends on: 20260714210000_promotions_engine_v1 (evaluate_cart_promotions, columns, tables)
...
BEGIN
  ...
  -- Index-friendly day window (idx_orders_store_created)
  ...
  -- Pass 1: resolve prices, validate stock, build write payload (no order yet)
  ...
  -- Promotions: evaluate after price/stock resolve, before tax/total
  ...
  -- Pass 2: write from resolved payload only (no price/product re-resolve)
  FOR v_resolved_line IN SELECT * FROM jsonb_array_elements(v_resolved)
  LOOP
  ...
  -- Persist promotion applications + bump coupon usage
...
-- ---------------------------------------------------------------------------
-- complete_checkout_expired_override — pass coupon through
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- complete_checkout_split — pass coupon through
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- complete_checkout_split_expired_override — pass coupon through
-- ---------------------------------------------------------------------------
