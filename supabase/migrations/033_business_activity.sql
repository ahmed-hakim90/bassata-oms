-- Business activity modes: supermarket weight sales, wholesale tiers, fractional order lines

CREATE TYPE business_activity_type AS ENUM (
  'cafe', 'ice_cream', 'restaurant', 'supermarket', 'retail', 'wholesale', 'mixed'
);

CREATE TYPE sales_mode AS ENUM ('retail', 'wholesale');

CREATE TYPE product_sales_unit_type AS ENUM (
  'piece', 'weight', 'volume', 'pack', 'mixed'
);

CREATE TYPE variant_kind AS ENUM ('standard', 'weight_portion');

CREATE TYPE variant_price_mode AS ENUM ('calculate_from_unit_price', 'fixed_price');

CREATE TYPE weight_sale_input_mode AS ENUM ('by_weight', 'by_amount');

-- Products
ALTER TABLE products
  ADD COLUMN sales_unit_type product_sales_unit_type NOT NULL DEFAULT 'piece',
  ADD COLUMN sale_unit measurement_unit NOT NULL DEFAULT 'piece',
  ADD COLUMN allow_fractional_quantity BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN allow_price_input BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN wholesale_enabled BOOLEAN NOT NULL DEFAULT false;

UPDATE products SET sale_unit = unit WHERE sale_unit IS DISTINCT FROM unit;

-- Variants
ALTER TABLE product_variants
  ADD COLUMN variant_kind variant_kind NOT NULL DEFAULT 'standard',
  ADD COLUMN quantity_value NUMERIC(12,4),
  ADD COLUMN quantity_unit measurement_unit,
  ADD COLUMN price_mode variant_price_mode,
...
CREATE TABLE product_price_tiers (
  ...
  name TEXT NOT NULL,
  sale_mode sales_mode NOT NULL DEFAULT 'retail',
...
CREATE INDEX idx_price_tiers_product_mode ON product_price_tiers(product_id, sale_mode, active);

ALTER TABLE product_price_tiers ENABLE ROW LEVEL SECURITY;

CREATE POLICY product_price_tiers_org ON product_price_tiers FOR ALL
  USING (org_id = auth_org_id())
  WITH CHECK (org_id = auth_org_id());

-- Orders
ALTER TABLE orders
  ADD COLUMN sales_mode sales_mode NOT NULL DEFAULT 'retail',
  ADD COLUMN activity_type business_activity_type NOT NULL DEFAULT 'retail';

ALTER TABLE order_items
  ALTER COLUMN quantity TYPE NUMERIC(12,4) USING quantity::NUMERIC(12,4);

ALTER TABLE order_items
  ADD COLUMN sale_unit measurement_unit,
  ADD COLUMN base_quantity NUMERIC(12,4),
  ADD COLUMN sale_input_mode weight_sale_input_mode,
  ADD COLUMN tier_id UUID REFERENCES product_price_tiers(id) ON DELETE SET NULL,
  ADD COLUMN wholesale_applied BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN line_note TEXT;

-- Seed business_activity for existing orgs
INSERT INTO app_settings (org_id, key, value)
SELECT o.id, 'business_activity', jsonb_build_object(
...
WHERE NOT EXISTS (
  ...
  WHERE a.org_id = o.id AND a.key = 'business_activity'
);

-- Merge new feature flags into existing rows
UPDATE app_settings
...
INSERT INTO permissions (key, label, description, group_name) VALUES
  ('wholesale_sale', 'Wholesale sale', 'Complete POS sales in wholesale mode', 'pos'),
  ('price_by_amount_sale', 'Sell by amount', 'Enter customer amount for weight products', 'pos'),
  ('weight_sale', 'Weight sale', 'Sell products by weight at POS', 'pos'),
  ('manage_business_activity', 'Manage business activity', 'Configure business activity modes', 'system'),
  ('manage_price_tiers', 'Manage price tiers', 'Create and edit product price tiers', 'products')
...
BEGIN
  FOR r_org IN SELECT id FROM organizations LOOP
    ...
    WHERE key IN (
      'wholesale_sale', 'price_by_amount_sale', 'weight_sale',
      'manage_business_activity', 'manage_price_tiers'
    )
    ON CONFLICT DO NOTHING;

    INSERT INTO role_permissions (org_id, role, permission_key) VALUES
      (r_org.id, 'manager', 'wholesale_sale'),
      (r_org.id, 'manager', 'price_by_amount_sale'),
      (r_org.id, 'manager', 'weight_sale'),
      (r_org.id, 'manager', 'manage_business_activity'),
      (r_org.id, 'manager', 'manage_price_tiers')
    ON CONFLICT DO NOTHING;
  END LOOP;
END $$;
...
GRANT EXECUTE ON FUNCTION complete_checkout(
