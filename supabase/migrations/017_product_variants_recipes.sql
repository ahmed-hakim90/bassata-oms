-- Product variants: absolute price, image, per-variant recipes, checkout RPC update
...
ALTER TABLE product_recipes DROP CONSTRAINT IF EXISTS product_recipes_product_id_key;

CREATE UNIQUE INDEX IF NOT EXISTS product_recipes_product_variant_unique
  ON product_recipes (product_id, COALESCE(variant_id, '00000000-0000-0000-0000-000000000000'::uuid));

CREATE OR REPLACE FUNCTION resolve_product_recipe_id(
  p_product_id UUID,
  p_variant_id UUID
) RETURNS UUID AS $$
DECLARE
...
BEGIN
  ...
  WHERE product_id = p_product_id
    AND (
      variant_id IS NOT DISTINCT FROM p_variant_id
      OR (p_variant_id IS NOT NULL AND variant_id IS NULL)
      OR (p_variant_id IS NULL AND variant_id IS NULL)
    )
  ORDER BY
    CASE WHEN variant_id IS NOT DISTINCT FROM p_variant_id THEN 0 ELSE 1 END
  LIMIT 1;
  RETURN v_recipe_id;
END;
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      COALESCE(pv.price, p.base_price + COALESCE(pv.price_delta, 0)),
      p.track_inventory,
      p.name,
      p.product_type
    INTO v_unit_price, v_track, v_product_name, v_product_type
    FROM products p
    LEFT JOIN product_variants pv ON pv.id = v_variant_id AND pv.product_id = p.id
...
GRANT EXECUTE ON FUNCTION resolve_product_recipe_id TO authenticated;
