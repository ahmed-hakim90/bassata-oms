-- Sales invoices (wholesale document flow): draft → issued → delivered
-- Independent of cashier session; stock + money mutate only on deliver (ADR-007).

CREATE TYPE public.sales_document_status AS ENUM ('draft', 'issued', 'delivered');

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS document_status public.sales_document_status,
  ADD COLUMN IF NOT EXISTS issued_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS warehouse_id UUID REFERENCES public.warehouses(id);

COMMENT ON COLUMN public.orders.document_status IS
  'Back-office sales invoice lifecycle. NULL = POS/online checkout order (not a document invoice).';
COMMENT ON COLUMN public.orders.warehouse_id IS
  'Warehouse used for sales-invoice stock deduction on deliver.';

CREATE INDEX IF NOT EXISTS idx_orders_store_document_status
  ON public.orders (store_id, document_status)
  WHERE document_status IS NOT NULL;

-- ---------------------------------------------------------------------------
-- issue_sales_invoice: draft → issued (no stock / no payment)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Order not found'; END IF;
  IF NOT has_store_access(v_order.store_id) THEN RAISE EXCEPTION 'Store access denied'; END IF;
  IF is_period_closed(v_order.store_id, now()) THEN
  ...
  LOOP
    ...
    ELSIF COALESCE(v_track, false) THEN
      ...
      INSERT INTO inventory_movements (
        ...
        'sale', -v_base_qty, 'order', p_order_id, v_caller
      );
    END IF;
  END LOOP;
  ...
  ELSIF p_payment_method = 'credit' THEN
    ...
    INSERT INTO customer_ledger (
      ...
      v_order.total, 0, p_order_id, v_order.order_number, v_caller
    );
    UPDATE customers
